...
void AES_Init(AES_InitTypeDef* AES_InitStruct)
{
  ...
  ASSERT(IS_AES_MODE(AES_InitStruct->AES_Operation));
  ASSERT(IS_AES_CHAINING(AES_InitStruct->AES_Chaining));
  ASSERT(IS_AES_DATATYPE(AES_InitStruct->AES_DataType));
...
{
  /* Check the parameter */
  ASSERT(IS_AES_DMA_TRANSFER(AES_DMATransfer));
